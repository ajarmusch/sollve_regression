...
int test_omp_thread_kernel(void)
{
    ...
    OMPREG_TEST_AND_SET_VERBOSE(errors, expect_total != total);
    printf("Total from loop directive is %d but expected total is %d.\n", total, expect_total);
    OMPREG_ERROR_IF(expect_total != total, "Total from loop directive is %d but expected total is %d.", total, expect_total);
...
int main(void)
{
    OMPREG_TEST_OFFLOADING;
    int errors = 0;

    OMPREG_TEST_AND_SET_VERBOSE(errors, test_omp_thread_kernel() != 0);

    // No error will be reported even if it is recorded.
    OMPREG_REPORT_AND_RETURN(errors);
}